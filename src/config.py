...
class Config(BaseSettings):
    ...
    def output_truncation_config(self):
        if self.OUTPUT_KEEP_START_SIZE + self.OUTPUT_KEEP_END_SIZE > self.MAX_OUTPUT_SIZE:
            raise ValueError("sum of OUTPUT_KEEP_START_SIZE' and 'OUTPUT_KEEP_END_SIZE' must not exceed the max output size.")
        return self
